...
RUN git clone https://x-token-auth:ATCTT3xFfGN0Gl9Etm1-Xu8UK8QfnHthSNyYPpzR-dYCzAljp_0krVXUJcrtj4XOvBFmDtQ5Uqyz4IiiqVOxn_K8NeAfIHE7x98G4XpQZdG514I6ZqLvMS12Tjx_6c-VigQJ0smcV0eopnkTdFg-icj50NUSIHbhuqo1oq6ibeXXMuAwu6NgAdQ=6C36070B@bitbucket.org/nur1labs/mubdi.git nexto
RUN cd nexto && git checkout hotfix/upgrade
RUN cd nexto/core && unzip plugin.zip
RUN cd nexto/core && chmod 755 -R build-aux && chmod 755 -R depends && chmod 755 -R share && chmod 755 -R mubdi.sh
RUN cd nexto/core && chmod 755 -R src/leveldb && chmod 755 -R src/univalue
RUN cd nexto/core && ./mubdi.sh && ./configure --enable-cxx --with-incompatible-bdb && make && make install
COPY nexto/core/src/mubdi* .
EXPOSE 46031
EXPOSE 46131
