...
const char * const MUBDI_CONF_FILENAME = "nexto.conf";
const char * const MUBDI_PID_FILENAME = "nexto.pid";
const char * const MUBDI_MASTERNODE_CONF_FILENAME = "nexto-mn.conf";
...
public:
    ...
    static inline std::pair<bool,std::string> GetArg(const ArgsManager& am, const std::string& arg)
    {
        ...
        // argument value seen from the command line (so "nextod -foo=bar
...
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "NEXT";
#else
    fs::path pathRet;
