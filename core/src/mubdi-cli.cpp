...
std::string HelpMessageCli()
{
    ...
    strUsage += HelpMessageOpt("-rpcwallet=<walletname>", "Send RPC for non-default wallet on RPC server (needs to exactly match corresponding -wallet option passed to nextod)");
...
static bool AppInitRPC(int argc, char* argv[])
{
    ...
    if (argc < 2 || gArgs.IsArgSet("-?") || gArgs.IsArgSet("-h") || gArgs.IsArgSet("-help") || gArgs.IsArgSet("-version")) {
        ...
        if (!gArgs.IsArgSet("-version")) {
            strUsage += "\n"
                        "Usage:  nexto-cli [options] <command> [params]  Send command to " PACKAGE_NAME "\n"
                        "or:     nexto-cli [options] -named <command> [name=value]... Send command to " PACKAGE_NAME " (with named arguments)\n"
                        "or:     nexto-cli [options] help                List commands\n"
                        "or:     nexto-cli [options] help <command>      Get help for a command\n";
            strUsage += "\n" + HelpMessageCli();
        }
