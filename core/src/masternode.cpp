                                  ...
                                  int chainHeight)
{
    ...
    if (!fOffline && !masternodeSync.IsBlockchainSynced()) {
        ...
        LogPrint(AILog::MASTERNODE,"CMasternodeBroadcast::Create -- %s\n", strErrorRet);
        return false;
    }
                                  ...
                                  CMasternodeBroadcast& mnbRet)
{
    ...
    LogPrint(AILog::MASTERNODE, "CMasternodeBroadcast::Create -- pubKeyCollateralAddressNew = %s, pubKeyMasternodeNew.GetID() = %s\n",
...
bool CMasternodeBroadcast::CheckAndUpdate(int& nDos)
{
    ...
    if (sigTime > GetMaxTimeWindow()) {
        LogPrint(AILog::MASTERNODE,"mnb - Signature rejected, too far into the future %s\n", vin.prevout.hash.ToString());
        nDos = 1;
        return false;
    }
    ...
    if (nMessVersion != MessageVersion::MESS_VER_HASH) {
        LogPrint(AILog::MASTERNODE, "mnb - rejecting old message version for mn %s\n", vin.prevout.hash.ToString());
        nDos = 30;
        return false;
    }

    if (protocolVersion < ActiveProtocol()) {
        LogPrint(AILog::MASTERNODE,"mnb - ignoring outdated Masternode %s protocol version %d\n", vin.prevout.hash.ToString(), protocolVersion);
        return false;
    }
    ...
    if (pubkeyScript.size() != 25) {
        LogPrint(AILog::MASTERNODE,"mnb - pubkey the wrong size\n");
        nDos = 100;
        return false;
    }
    ...
    if (pubkeyScript2.size() != 25) {
        LogPrint(AILog::MASTERNODE,"mnb - pubkey2 the wrong size\n");
        nDos = 100;
        return false;
    }

    if (!vin.scriptSig.empty()) {
        LogPrint(AILog::MASTERNODE,"mnb - Ignore Not Empty ScriptSig %s\n", vin.prevout.hash.ToString());
        return false;
    }
    ...
    if (Params().NetworkIDString() == CBaseChainParams::MAIN) {
        if (addr.GetPort() != 41031) return false;
    } else if (addr.GetPort() == 41031)
        return false;
    ...
    if (pmn->pubKeyCollateralAddress == pubKeyCollateralAddress && !pmn->IsBroadcastedWithin(MasternodeBroadcastSeconds())) {
        ...
        LogPrint(AILog::MASTERNODE,"mnb - Got updated entry for %s\n", vin.prevout.hash.ToString());
...
{
    if (sigTime > GetMaxTimeWindow()) {
        LogPrint(AILog::MNPING,"%s: Signature rejected, too far into the future %s\n", __func__, vin.prevout.hash.ToString());
        nDos = 30;
        return false;
    }

    if (sigTime <= GetAdjustedTime() - 60 * 60) {
        LogPrint(AILog::MNPING,"%s: Signature rejected, too far into the past %s - %d %d \n", __func__, vin.prevout.hash.ToString(), sigTime, GetAdjustedTime());
        nDos = 30;
        return false;
    }
    ...
    // reject old signature version
    if (nMessVersion != MessageVersion::MESS_VER_HASH) {
        LogPrint(AILog::MNPING, "mnp - rejecting old message version for mn %s\n", vin.prevout.hash.ToString());
        nDos = 30;
        return false;
    }
    ...
    if (!mnodeman.IsWithinDepth(blockHash, 2 * MNPING_DEPTH)) {
        LogPrint(AILog::MNPING,"%s: Masternode %s block hash %s is too old or has an invalid block hash\n",
    ...
    if (isMasternodeFound && pmn->protocolVersion >= ActiveProtocol()) {
        ...
        if (!pmn->IsPingedWithin(MasternodeMinPingSeconds() - 60, sigTime)) {
            ...
            LogPrint(AILog::MNPING, "%s: Masternode ping accepted, vin: %s\n", __func__, vin.prevout.hash.ToString());

            Relay();
            return true;
        }
        LogPrint(AILog::MNPING, "%s: Masternode ping arrived too early, vin: %s\n", __func__, vin.prevout.hash.ToString());
        //nDos = 1; //disable, this is happening frequently and causing banned peers
        return false;
    }
    LogPrint(AILog::MNPING, "%s: Couldn't find compatible Masternode entry, vin: %s\n", __func__, vin.prevout.hash.ToString());
